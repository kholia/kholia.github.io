...
[Tangara](https://www.crowdsupply.com/cool-tech-zone/tangara) is an excellent
project!
...
We then even briefly considered using an ESP32-WROVER-E-N16R8 module (just like Tangara) to reuse the Tangara's firmware. However, we were NOT very excited about doing that.
...
Power source: We may need to use two 14500 Li-ion AA batteries (we need 5V). Or
perhaps we can get away with one 14500 Li-ion cell, since the LicheeRV Nano's
operating range is `3.6~5.5V`. This will be verified experimentally.
...
I see the `Audiocular D07` DAC in action while playing the FLAC files from the [Oppo Digital](https://www.oppodigital.com/hra/dsd-by-davidelias.aspx) webpage.
