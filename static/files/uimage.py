...
def load_file(li, neflags, format):
    ...
    if format.startswith(RomFormatName):
        ...
        if not header.ih_comp in (IH_COMP_NONE, IH_COMP_GZIP, IH_COMP_LZMA):
          idc.warning("Can only handle uncompressed, gzip-compressed, or lzma-compressed images")
          return
        ...
        elif header.ih_comp == IH_COMP_GZIP:
        ...
        elif header.ih_comp == IH_COMP_LZMA:
          cdata = li.read(header.ih_size)
          # U-Boot uses raw LZMA streams without end-of-stream markers
          # Use LZMADecompressor with FORMAT_ALONE for raw LZMA streams
          try:
            decompressor = lzma.LZMADecompressor(format=lzma.FORMAT_ALONE)
            udata = decompressor.decompress(cdata)
          except lzma.LZMAError as e:
            idc.warning("LZMA decompression failed: %s" % str(e))
            return
          # expand segment to fit uncompressed data
          idc.set_segment_bounds(header.ih_load, header.ih_load, header.ih_load+len(udata), idc.SEGMOD_KEEP)
          idaapi.put_bytes(header.ih_load, udata)
